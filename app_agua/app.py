...
def debug():
    ...
    return str(data)
# =================================================================
# 🔍 VISTA 3: Buscador e Historial con Filtros leyendo Supabase (GET)
# =================================================================
...
def historial_auditoria():
    ...
    try:
        ...
        if filtro_cajera:
            query += " AND cajera LIKE %s"
            params.append(f"%{filtro_cajera}%")
        if filtro_carnicero:
            query += " AND carnicero LIKE %s"
        ...
        cur.close()
        con.close()
        print("☁️ Historial de auditoría extraído con éxito desde Supabase.")
...
def reporte_ventas():
    ...
    for fila in ventas_departamento_crudas:
        ...
        costo = 0.0
        if depto == "Verdulería":
            costo = 2000.0  # Costo fijo para que tu ejemplo de $3,146 coincida
        elif depto == "Cigarrillos":
            costo = 0.0     # Cigarrillos se mantiene en 0.0 para evaluar la protección
 
        # Cálculo de la ganancia real neta por departamento
        ganancia = venta - costo
 
        # PROTECCIÓN CONTRA DIVISIÓN POR CERO (Corrige el error de margen 0% en Cigarrillos)
        if costo == 0.0:
            margen = 100.0 if venta > 0 else 0.0
        else:
            margen = round((ganancia / costo) * 100, 1)
 
        # Añadimos la información estructurada como diccionario para la plantilla HTML
...
# =================================================================
# 🥩 VISTA 1: Formulario de Carga Mayorista (GET)
# =================================================================
@app.route('/compras/nuevo', methods=['GET'])
def nuevo_ingreso_hacienda():
    # Renderiza la interfaz limpia para cargar la media res
    return render_template('formulario_ingreso.html', admin=True)


# =================================================================
# 🥩 PROCESADOR 1: Guardar la Media Res directo en Supabase (POST)
# =================================================================
...
def guardar_ingreso_hacienda():
    ...
    try:
        ...
        cur.close()
        con.close()
        print("☁️ Compra mayorista guardada con éxito en Supabase.")
        
    except Exception as e:
        print(f"❌ Error crítico en Supabase al guardar compra: {e}")
...
# =================================================================
# 🥩 PROCESADOR 2: Cargar Kilos por Corte directo en Supabase (POST)
# =================================================================
...
def cargar_inventario_cortes():
    ...
    try:
        ...
        cur.close()
        con.close()
        print(f"☁️ Replicación exitosa para el corte {nombre_corte} con responsables.")
        
    except Exception as e:
        print(f"❌ Error crítico en Supabase al cargar corte: {e}")
        return f"Error de conexión con la nube: {e}", 500
...
# =================================================================
# 🥩 VISTA 2: El Panel de Control Diario leyendo Supabase (GET)
# =================================================================
@app.route('/auditoria/balance-diario', methods=['GET'])
def balance_diario():
    fecha_filtro = request.args.get('fecha', '2026-07-01')
    
    # ☁️ Lectura directa desde Supabase usando RealDictCursor
    try:
        con = get_db_cloud() 
        cur = con.cursor(cursor_factory=RealDictCursor)
        ...
        print(f"❌ Error de lectura cloud en balance diario: {e}")
        cortes_del_dia = []
    
    # 🔴 ALGORITMO "SUMAR.SI" FINANCIERO AUTOMATIZADO (<0)
    total_fuga_dinero = 0
    errores_facturacion_positivos = 0
    
    for corte in cortes_del_dia:
        dinero_perdido = float(corte['dinero_perdido'] or 0)
        
        if dinero_perdido < 0:
            total_fuga_dinero += dinero_perdido
        elif dinero_perdido > 0:
            errores_facturacion_positivos += dinero_perdido
    
    # Renderizamos la plantilla inyectando los datos de Supabase
